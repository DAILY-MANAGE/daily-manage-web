...
export default function FormSubmitter({ data, params }: FormSubmitterProps) {
  ...
  const midtermSubmit = async (data: typeof defaultForm) => {
    ...
    const submitData = {
      respostas: data.respostas.filter((data: InnerData | undefined) => data?.idpergunta)
    }
          ...
          data && data.data && (
            <>
              {data.data.perguntas.map((formData: FormQuestion, index: number) => {
                ...
                return <Card key={index} className={hasError ? "outline outline-offset-1 outline-2 outline-red-600" : ''}>
                  <CardHeader className='pb-2'>
                    <Label>{formData.descricao} {!formData.opcional && (<span className='text-xs text-red-900'>(obrigatório)</span>)}</Label>
                  </CardHeader>
                  <CardContent>
                    <FormResponse formData={formData} errors={errors} register={register} getValues={getValues} setValue={setValue} hasError={hasError}/>
                  </CardContent>
                </Card>
              })}
            </>
